<template>
  <div>
    ...
    <span class="box span" @click="router.push(`/manager/buildingmanagement/buildings?sex=${sex}`)">栋</span>
  </div>
  <div class="box" v-for="building in buildings" @click="router.push(`/manager/buildingmanagement/units?sex=${sex}&building=${building}`)">{{ building }}</div>
...
const add=()=>{
  let building=prompt("请输入栋号")
  if(building){
       let dormitory = {
          did:building + "01" + "01" + "01",
          building: building,
          unit: "01",
          floor:'01',
          room:"01",
