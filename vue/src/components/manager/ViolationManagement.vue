<template>
  <div>
    <div class="topTool">
      ...
      <el-button type="primary" :icon="Plus" @click="handleAdd" style="margin-left: 20px;">添加违纪信息</el-button>
...
//获取当前页数据
const getData = () => {
  axios.get('http://localhost:8080/violation/findByPage', {
    params: {
...
const delrow = (row) => {
  const vid = row.vid;
  axios.delete(`http://localhost:8080/violations/${vid}`)
      .then(response => {
        // 处理删除成功后的逻辑，例如重新加载数据
        getData();
      })
