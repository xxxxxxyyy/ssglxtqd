<template>
  <div>
    ...
    >
      <!--el-table-column列-->
      <el-table-column prop="vid" label="违纪记录号" width="150px" align="center" header-align="center" ></el-table-column>
      <el-table-column prop="sid" label="学号" width="100px" align="center" header-align="center" ></el-table-column>
      <el-table-column prop="did" label="宿舍号" width="100px" align="center" header-align="center"></el-table-column>
      <el-table-column prop="description" label="描述" width="150px" align="center" header-align="center"></el-table-column>
      <el-table-column prop="type" label="处罚方法" width="100px" align="center" header-align="center" ></el-table-column>
      ...
      <el-table-column label="操作" min-width="300px" align="center" header-align="center">
        <template #default="{ row }">
          <el-button type="warning" size="small" :icon="Warning" @click="handleDelete(row)" v-if="row.state === 0">申报</el-button>
          <el-button type="info" size="small" :icon="Warning" v-if="row.state === 1" disabled>已申报</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialogFormVisible" :title="dialogTitle" style="width:600px">
      <el-form :model="tableform">
        ...
        <el-form-item label="宿舍号" :label-width="200">
          <el-input v-model="tableform.did" autocomplete="off" style="width:220px"/>
        </el-form-item>
        <el-form-item label="描述" :label-width="200">
          <el-input v-model="tableform.description" autocomplete="off" style="width:220px"/>
        </el-form-item>
        <el-form-item label="处罚" :label-width="200">
          <el-input v-model="tableform.type" autocomplete="off" style="width:220px"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { Plus,Delete,Warning,Search } from '@element-plus/icons-vue'
//定义用户信息数据
...
const getData = () => {
  ...
  axios.get(`http://localhost:8080/violation/findByPage1/${sid}`, {
    params: {
...
const handleSearch = (keyword) => {
  if(keyword.length>0){
    axios.get(`http://localhost:8080/violationByKeyword?keyword=${keyword}&pageNum=${currentPage.value}&pageSize=${pageSize.value}`).then(response=>{
      data.value = response.data.records;
      total.value = response.data.total;
    }).catch(error=>{
      console.error(error)
...
const dialogOk = () => {
  ...
  if (dialogType.value === 'add') {
    // 获得新的违纪记录号
    axios.get('http://localhost:8080/violationvid').then(response=>{
      const newUser = {
        ...tableform.value,
        vid: response.data,
        date: new Date(),
      ...
      console.log(newUser)
      axios.post('http://localhost:8080/violations', newUser)
          .then(response => {
    ...
    axios.put(`http://localhost:8080/violations/${vid}`, updatedUser)
        .then(response => {
          // 处理成功响应
...
const handleDelList = () => {
  ElMessageBox.confirm(
    ...
    axios.delete('http://localhost:8080/violations', {data:vids})
        .then(response => {
          getData();
