<template>
  <div>
    <div class="topTool">
      ...
      <el-button type="primary" :icon="Plus" @click="handleAdd" style="margin-left: 20px;">缴费</el-button>
    </div>
    ...
    >
      ...
      <el-table-column prop="sid" label="学号" width="100px" align="center" header-align="center" disabled></el-table-column>
      ...
      <el-table-column label="操作" min-width="300px" align="center" header-align="center">
        <template #default="{ row }">
          <el-button type="danger" size="small" :icon="Wallet" @click="handleDelete(row)" v-if="row.sum>0&&row.state===0">退费</el-button>
          <el-button type="info" size="small" :icon="Wallet" @click="handleDelete(row)" v-if="row.sum<0&&row.state===1" disabled>已退费</el-button>
          <el-button type="primary" size="small" :icon="Wallet" v-if="row.sum>0&&row.state===1" disabled>待审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialogFormVisible" :title="dialogTitle" style="width:600px">
      <el-form :model="tableform">
        <el-form-item label="学号" :label-width="200">
          <el-input v-model="tableform.sid" autocomplete="off" v-if="dialogType === 'add'" style="width:220px " disabled/>
...
const getData = () => {
  ...
  console.log(sid)
  axios.get(`http://localhost:8080/expense/findByPage1/${sid}`, {
    params: {
...
const handleAdd = () => {
  ...
  tableform.value = {"sid":localStorage.getItem("uid")}//清空表单数据
}
